...
export function resolveStepContent(step: RecipeStep, ingredientsById: Map<string, Ingredient>): string {
  return step.content.replace(/\{(\w+)\}/g, (match, id: string) => {
    const ingredient = ingredientsById.get(id)
    if (!ingredient) return match
    return formatQuantity(ingredient.amount, ingredient.unit)
  })
}
